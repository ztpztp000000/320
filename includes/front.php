 ...
 * 前台公用
*/
function smarty_header($cache=false, $site=''){
	if (empty($site))
	{
		$GLOBALS['smarty']->template_dir	=ROOT_PATH.'templates/'.$GLOBALS['config']['site_template'];
		$GLOBALS['smarty']->cache_dir		=ROOT_PATH.'temps/website';
		$GLOBALS['smarty']->compile_dir	=ROOT_PATH.'temps/website';
	}
	else
	{
		$GLOBALS['smarty']->template_dir	=ROOT_PATH.'templates/'.$site;
		$GLOBALS['smarty']->cache_dir		=ROOT_PATH.'temps/'.$site;
		$GLOBALS['smarty']->compile_dir	=ROOT_PATH.'temps/'.$site;
	}
	if($cache){
...
function message($message=array()){
	...
	$smarty->cache_dir		=ROOT_PATH.'temps/website';
	$smarty->compile_dir	=ROOT_PATH.'temps/website';
...
function insert_get_cur_money(){
	if(check_login()){
		return get_user_vc($_SESSION['member_username']);
	}
	else{
		return '0';
	}
