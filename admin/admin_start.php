...
if($action==''){
	...
	$reg_info = array();
	// 总注册人数
	$sql = "SELECT COUNT(*) FROM ".$db_prefix."member";
	$row = $GLOBALS['db']->getone($sql);
	$reg_info['total_user'] = $row[0];
	// 本月注册人数
	$sql = "SELECT COUNT(*) FROM ".$db_prefix."member WHERE DATE_FORMAT(FROM_UNIXTIME(member_join_time),'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')";
	$row = $GLOBALS['db']->getone($sql);
	$reg_info['month_reg'] = $row[0];
	// 本日注册人数
	$sql = "SELECT COUNT(*) FROM ".$db_prefix."member WHERE DATE_FORMAT(FROM_UNIXTIME(member_join_time),'%Y-%m-%d')=DATE_FORMAT(NOW(),'%Y-%m-%d')";
	$row = $GLOBALS['db']->getone($sql);
	$reg_info['day_reg'] = $row[0];
	// 总充值金额
	$sql = "SELECT sum(charge_money) FROM ".$db_prefix."pay WHERE pay_state=2";
	$row = $GLOBALS['db']->getone($sql);
	$reg_info['total_pay'] = $row[0];
	// 本月充值金额
	$sql = "SELECT sum(charge_money) FROM ".$db_prefix."pay WHERE pay_state=2 and DATE_FORMAT(FROM_UNIXTIME(pay_time),'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')";
	$row = $GLOBALS['db']->getone($sql);
	$reg_info['month_pay'] = $row[0];
	// 今日充值金额
	$sql = "SELECT sum(charge_money) FROM ".$db_prefix."pay WHERE pay_state=2 and DATE_FORMAT(FROM_UNIXTIME(pay_time),'%Y-%m-%d')=DATE_FORMAT(NOW(),'%Y-%m-%d')";
	$row = $GLOBALS['db']->getone($sql);
	$reg_info['day_pay'] = $row[0];
	
	// 最新服务器
	$server_list = get_new_server();
	$new_server = array();
	$i = 0;
	foreach($server_list as $server)
	{
		$new_server[$server['id']]['id'] = $server['id'];
		$new_server[$server['id']]['name'] = $server['game_name']."-".$server['name'];
		$sql = "SELECT sum(charge_money) FROM ".$db_prefix."pay WHERE pay_state=2 AND pay_server_id=".$server['id'];
		$row = $GLOBALS['db']->getone($sql);
		$new_server[$server['id']]['pay'] = empty($row[0])?0:$row[0];
		$i++;
		if (i == 5)
		{
			break;
	...
	$smarty->assign('reg_info',$reg_info);
	$smarty->assign('server_list',$new_server);
