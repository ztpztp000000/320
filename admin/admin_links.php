...
if($do==''){
	...
	$link_list=array();
	$sql="SELECT * FROM ".$db_prefix."link where game_id=0 OR is_show_home=1";
	$page_size=30;
	...
	if($count>0){
			...
			$pagebar=pagebar(get_self(),"sort=game_flat&action=links&",$page_current,$page_size,$count);
	}else{
			$pagebar="";
	}
...
if($do=='link_insert'){
	...
	clear_cache();
	message(array('text'=>$language['link_insert_is_success'],'link'=>'?sort=game_flat&action=links'));
}
if($do=='link_edit'){
	check_permissions('link_write');
...
if($do=='link_update'){
	...
	admin_log('update','link',$link_name);
	clear_cache();
	message(array('text'=>$language['link_update_is_success'],'link'=>'?sort=game_flat&action=links'));
}
if($do=='link_delete'){
	...
	clear_cache();
	message(array('text'=>$language['link_delete_is_success'],'link'=>'?sort=game_flat&action=links'));
}
?>